...
class LegionSolver {
  ...
  constructor(board, pieces, onBoardUpdated) {
    ...
    this.middle = [];
    for (
      let i = this.board.length / 2 - 1;
      i < this.board.length / 2 + 1;
      i++
    ) {
      for (
        let j = this.board[0].length / 2 - 1;
      ...
      if (
        ...
        if (this.restrictedPieceNumber != this.pieceLength) {
          ...
          if (this.isPlaceable(point, piece)) {
            stack.push([
              0,
              0,
              this.takeFromList(this.restrictedPieceNumber),
              [...this.restrictedSpots],
              point,
        ...
        if (this.isPlaceable(point, piece)) {
          ...
          stack.push([
            this.pieceNumber,
            this.transformationNumber,
            this.takeFromList(this.pieceNumber),
            [...this.restrictedSpots],
            point,
            0,
            0,
  ...
  takeFromList(placement) {
    ...
    while (
      fill.amount * fill.cellCount <
      this.pieces[index].amount * this.pieces[index].cellCount
    )
      index++;
  ...
  isValid() {
    ...
    for (let point of this.middle) {
      if (
        ...
        this.board[point.y][point.x] <= this.pieceLength
      ) {
        normalPieces++;
      }
    }
  ...
  searchSurroundings(x, y) {
    ...
    if (this.board[y] && this.board[y][x] == 0) {
      ...
      if (restrictedSpaces <= 1) {
        this.restrictedSpots.push(
          new RestrictedPoint(x, y, 4 - restrictedSpaces)
        );
      }
    }
  }
  ...
  changeIndex(restricted) {
    if (restricted) {
      ...
      if (
        this.transformationNumber <
        this.pieces[this.pieceNumber].transformations.length - 1
      ) {
        this.transformationNumber++;
      } else {
        this.pieceNumber++;
